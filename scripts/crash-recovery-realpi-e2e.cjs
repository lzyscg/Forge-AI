...
//   - PI_MODE=real，真实 DeepSeek 模型调用
//   - 真正的 taskkill /F /T（SIGKILL 等价），不是 MAX_TURNS 干净退出
...
const DB = process.env.CRASH_DB || './data/crash-realpi.db';
const SESSION_DIR = process.env.CRASH_SESSION_DIR || './data/crash-realpi-sessions';
const SCENARIO = process.env.SCENARIO_PATH || './scenarios/songwriting/scenario.yaml';
const MODEL = process.env.PI_MODEL_ID || 'deepseek-v4-flash';
const KILL_AFTER_TURNS = parseInt(process.env.KILL_AFTER_TURNS || '2', 10); // 达到 N 个 completed turn 后 kill
...
/** 数 persistent session .jsonl 的条目数（每个 sessionRef 目录正常只有 1 个 .jsonl）。 */
...
/** 取 post-resume 阶段（sequence > killSeq）的 persistent agent turn 的 context_snapshot.rendered_context。 */
...
(async () => {
  ...
  // ---------- Phase 1: real Pi，跑到 N 个 completed turn 后 taskkill /F ----------
  console.log(`=== Phase 1: real Pi 跑到 ${KILL_AFTER_TURNS} 个 completed turn 后 kill -9 ===`);
  const child1 = spawnWorker();
  ...
  // ---------- Phase 2: 重启同一 DB，续跑 ----------
  console.log(`\n=== Phase 2: 重启 worker（同一 DB + 同一 session dir），续跑 ===`);
  ...
  while (!phase2Done && Date.now() - phase2Start < PHASE2_DEADLINE_MS) {
    ...
    const d = openDB();
    let st = null;
    try { st = d.prepare("SELECT status FROM cases WHERE case_id=?").get(caseId)?.status; } finally { d.close(); }
    if (st === 'approved' || st === 'failed' || st === 'stopped' || st === 'waiting_human') {
      console.log(`  Case 终态: ${st}，kill worker 2`);
      killTree(child2.pid);
      break;
    }
  }
  if (!phase2Done) {
    console.log('  超时，kill worker 2');
    killTree(child2.pid);
  }
  await exitPromise;
  console.log('  worker 2 已退出');
  ...
  // 3. 从 Turn N+1 续跑（不重跑 1..N）：turn 数 > killSeq 且 Turn 1..N 的 output_message_id 不变
  ...
  // 7. 续跑后 persistent agent 的 context_snapshot 仍含崩溃前产物内容（2.3 "从 context_snapshot 可验证"）
  let ctxHasPrior = false;
  const postCtxs = postResumeRenderedContexts(killSeq);
  // 用崩溃前最后一个产物版本的内容片段作为"历史内容"标志
